...
def get_related_books(book_id, limit=10, offset=0):
    book = calibre_db.get_book(book_id)
    if not book:
        return []

    related_books_query = calibre_db.session.query(db.Books)

    # Filter by the same author
    if book.authors:
        related_books_query = related_books_query.filter(db.Books.authors.any(db.Authors.id.in_([author.id for author in book.authors])))

    # Filter by the same categories
    if book.tags:
        related_books_query = related_books_query.filter(db.Books.tags.any(db.Tags.id.in_([tag.id for tag in book.tags])))

    # Filter by similar text in description
    if book.comments:
        comment_text = book.comments[0].text if book.comments else ""
        related_books_query = related_books_query.filter(db.Books.comments.ilike(f"%{comment_text}%"))

    # Exclude the current book from the results
    related_books_query = related_books_query.filter(db.Books.id != book_id)

    # Apply limit and offset for pagination
    related_books_query = related_books_query.limit(limit).offset(offset)

    return related_books_query.all()
...
# Modify the show_book function to include related books
...
def show_book(book_id):
    ...
    if entries:
        ...
        # Check if email_share_list is empty
        if not entry.email_share_list:
            log.debug("email_share_list is empty.")
        else:
            log.debug("email_share_list is not empty: {}".format(entry.email_share_list))

        # Fetch related books
        related_books = get_related_books(book_id)
...
# Add this route to handle loading more related books

@web.route("/ajax/load_more_related_books")
@login_required_if_no_ano
def load_more_related_books():
    book_id = request.args.get('book_id', type=int)
    offset = request.args.get('offset', type=int, default=10)
    related_books = get_related_books(book_id, limit=10, offset=offset)

    related_books_data = [{
        'id': book.id,
        'title': book.title,
        'author': book.ordered_authors[0].name,
        'cover_url': url_for('web.get_cover', book_id=book.id, resolution='md'),
        'url': url_for('web.show_book', book_id=book.id)
    } for book in related_books]

    return jsonify({'related_books': related_books_data})